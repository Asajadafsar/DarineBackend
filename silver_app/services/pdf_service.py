# silver_app/services/pdf_service.py - اصلاح نهایی
...
class SilverInvoicePDFService:
    """سرویس تولید PDF فاکتور نقره با دیزاین لاکچری طلایی/سرمه‌ای"""
    ...
    def generate_invoice_pdf(cls, invoice_id, request):
        try:
            ...
            logger.error(f"فاکتور نقره با id {invoice_id} یافت نشد")
            return None

        data = SilverInvoiceService.get_invoice_data(invoice)

        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="silver_invoice_{invoice.invoice_number}.pdf"'
        ...
        if 'خرید' in invoice_type or invoice_type == 'BUY':
            title_text = 'فاکتور خرید نقره'
        elif 'فروش' in invoice_type or invoice_type == 'SELL':
            title_text = 'فاکتور فروش نقره'
        else:
        ...
        # ============================================================
        # ✅ جدول اطلاعات خریدار (فروشنده فقط نام و آدرس)
        # ============================================================
        ...
        info_data = [
            ...
            [
                ...
                Paragraph(cls.fix_persian_text(""), rtl_style),  # ❌ خالی
            ],
            [
        ...
        # جدول جزئیات نقره
        # ============================================================
        silver_header_style = ParagraphStyle(
            'SH', parent=styles['Normal'], fontName=font_bold, fontSize=9.2,
            alignment=TA_CENTER, textColor=cls.WHITE, leading=13,
        )
        silver_value_style = ParagraphStyle(
            'SV', parent=styles['Normal'], fontName=font_bold, fontSize=11,
            alignment=TA_CENTER, textColor=cls.INK, leading=15,
        )
        silver_summary_style = ParagraphStyle(
            'SS', parent=styles['Normal'], fontName=font_name, fontSize=9,
            alignment=TA_CENTER, textColor=cls.INK, leading=13,
        )
        silver_total_label_style = ParagraphStyle(
            'STL', parent=styles['Normal'], fontName=font_name, fontSize=9.5,
            alignment=TA_CENTER, textColor=cls.GOLD_LIGHT, leading=13,
        )
        silver_total_style = ParagraphStyle(
            'ST', parent=styles['Normal'], fontName=font_bold, fontSize=15,
            alignment=TA_CENTER, textColor=cls.GOLD_LIGHT, leading=19,
        )

        silver_weight = data.get('silver_weight_display', data.get('silver_weight', '0'))
        silver_price = data.get('silver_price_per_gram_display', data.get('silver_price_per_gram', '0'))
        pure_price = data.get('pure_silver_price_display', data.get('pure_silver_price', '0'))
        ...
        silver_data = [
            [
                Paragraph(cls.fix_persian_text('وزن (گرم)'), silver_header_style),
                Paragraph(cls.fix_persian_text('قیمت هر گرم'), silver_header_style),
                Paragraph(cls.fix_persian_text('قیمت خالص'), silver_header_style),
                Paragraph(cls.fix_persian_text('مبلغ نهایی'), silver_header_style),
            ],
            [
                Paragraph(cls.fix_persian_text(str(silver_weight)), silver_value_style),
                Paragraph(cls.fix_persian_text(str(silver_price)), silver_value_style),
                Paragraph(cls.fix_persian_text(str(pure_price)), silver_value_style),
                Paragraph(cls.fix_persian_text(str(total_amount)), silver_value_style),
            ],
            [
                Paragraph(
                    cls.fix_persian_text(f"کارمزد ({fee_rate}%): {fee_amount}"),
                    silver_summary_style,
                ),
                '',
                Paragraph(cls.fix_persian_text('مبلغ کل قابل پرداخت'), silver_total_label_style),
                Paragraph(cls.fix_persian_text(str(total_amount)), silver_total_style),
            ],
        ]

        col_w = CONTENT_WIDTH / 4
        silver_table = Table(silver_data, colWidths=[col_w, col_w, col_w * 0.85, col_w * 1.15])
        ...
        gw, gh = silver_table.wrap(CONTENT_WIDTH, height)
        y -= gh
        silver_table.drawOn(c, CONTENT_LEFT, y)
        ...
        c.setFillColor(cls.GRAY)
        c.drawCentredString(width / 2, y, cls.fix_persian_text("این فاکتور به عنوان مدرک معتبر خرید/فروش نقره محسوب می‌شود"))

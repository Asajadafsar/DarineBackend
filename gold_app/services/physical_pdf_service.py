# gold_app/services/physical_pdf_service.py
...
class PhysicalOrderInvoicePDFService:
    """سرویس تولید PDF فاکتور سفارش فیزیکی با دیزاین لاکچری طلایی/سرمه‌ای"""
    ...
    def generate_invoice_pdf(cls, invoice_id, request):
        try:
            invoice = PhysicalOrderInvoice.objects.get(id=invoice_id, order__user=request.user)
        ...
        response['Content-Disposition'] = f'attachment; filename="physical_invoice_{invoice.invoice_number}.pdf"'
        ...
        if 'خرید' in invoice_type or invoice_type == 'BUY':
            title_text = 'فاکتور خرید فیزیکی'
        elif 'فروش' in invoice_type or invoice_type == 'SELL':
            title_text = 'فاکتور فروش فیزیکی'
        else:
            title_text = 'فاکتور سفارش فیزیکی'
        ...
        buyer_postal = data.get('buyer_postal_code', '---')
        ...
        info_data = [
            ...
            [
                Paragraph(cls.fix_persian_text(f"شهر: {buyer_city} | کد پستی: {buyer_postal}"), rtl_style),
                Paragraph(cls.fix_persian_text(""), rtl_style),
            ],
        ]
        ...
        payment_method = data.get('payment_method_display', '---')
        ...
        gold_data = [
            ...
            [
                Paragraph(
                    cls.fix_persian_text(f"روش پرداخت: {payment_method}"),
                    gold_summary_style,
                ),
                '',
        ...
        if data.get('products'):
            product_header_style = ParagraphStyle(
                'PH', parent=styles['Normal'], fontName=font_bold, fontSize=8.5,
                alignment=TA_CENTER, textColor=cls.WHITE, leading=12,
            )
            product_value_style = ParagraphStyle(
                'PV', parent=styles['Normal'], fontName=font_name, fontSize=7.5,
                alignment=TA_CENTER, textColor=cls.INK, leading=11,
            )

            product_data = [
                [
                    Paragraph(cls.fix_persian_text('محصول'), product_header_style),
                    Paragraph(cls.fix_persian_text('تعداد'), product_header_style),
                    Paragraph(cls.fix_persian_text('وزن (گرم)'), product_header_style),
                    Paragraph(cls.fix_persian_text('قیمت'), product_header_style),
                ]
            ]

            for product in data.get('products', []):
                product_data.append([
                    Paragraph(cls.fix_persian_text(product.get('product_name', '---')), product_value_style),
                    Paragraph(cls.fix_persian_text(str(product.get('quantity', 0))), product_value_style),
                    Paragraph(cls.fix_persian_text(f"{product.get('total_weight', 0):.3f}"), product_value_style),
                    Paragraph(cls.fix_persian_text(f"{product.get('total_price', 0):,} تومان"), product_value_style),
                ])

            product_table = Table(product_data, colWidths=[CONTENT_WIDTH * 0.45, CONTENT_WIDTH * 0.15, CONTENT_WIDTH * 0.2, CONTENT_WIDTH * 0.2])
            ...
            pw, ph = product_table.wrap(CONTENT_WIDTH, height)
            y -= ph + 10 * mm
            product_table.drawOn(c, CONTENT_LEFT, y)
        ...
        c.drawCentredString(width / 2, y - 6.5 * mm, cls.fix_persian_text("کد رهگیری سفارش"))
        c.setFont(font_bold, 13)
        c.setFillColor(cls.GOLD_LIGHT)
        c.drawCentredString(width / 2, y - 14.5 * mm, cls.fix_persian_text(data.get('order_tracking_code', '---')))
